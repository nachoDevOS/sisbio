...
@section('titulo', 'Reporte de marcaciones procesadas')

@section('contenido')
    <div class="cabecera">
        <div class="cabecera__titulo">
            <span class="cabecera__icono"><x-heroicon-o-document-chart-bar /></span>
            <h1>Marcaciones procesadas</h1>
        </div>
    </div>
    ...
    <div x-data="{
             ...
             async generar() {
                 ...
                 this.generando = true;
                 try {
                     const params = new URLSearchParams({ persona: this.persona, desde: this.desde, hasta: this.hasta, print: 0 });
                     const resp = await fetch(`{{ route('reportes.marcaciones.procesado.generar') }}?${params.toString()}`, { headers: { 'Accept': 'text/html' } });
